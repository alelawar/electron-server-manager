...
export default function App() {
  ...
  // Tambah ini setelah useEffect localStorage
  useEffect(() => {
      ...
      const updated = await Promise.all(
        servers.map(async (server) => {
          try {
            ...
            return { ...server, status: isRunning ? "running" : "stopped" };
          } catch {
  ...
  const handleCommand = async (server, command) => {
    ...
    try {
      if (window.electronAPI) {
        ...
        if (command === "start")
          updateServerStatus(server.id, result.success ? "running" : "stopped");
        ...
        appendLog(
          server.id,
          `[mock] Command '${command}' executed on ${server.name}\n`,
        );
        ...
        <main className="flex-1 overflow-hidden">
          ...
          {activeView === "logs" && activeServer && (
            <LogViewer
              ...
              onClear={() =>
                setLogs((prev) => ({ ...prev, [activeServer.id]: [] }))
              }
